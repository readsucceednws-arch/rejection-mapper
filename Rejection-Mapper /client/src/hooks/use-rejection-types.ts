...
export function useBulkDeleteRejectionTypes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (ids: number[]) => {
      const res = await fetch("/api/rejection-types/bulk", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      if (!res.ok) {
        ...
        throw new Error((err as { message?: string }).message || "Failed to delete rejection reasons");
      }
      return res.json();
    },
    onSuccess: () => {
