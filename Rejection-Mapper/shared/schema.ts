...
export const rejectionEntriesRelations = relations(rejectionEntries, ({ one }) => ({
  ...
  createdBy: one(users, { fields: [rejectionEntries.createdByUserId], references: [users.id] }),
}));
...
export const reworkEntriesRelations = relations(reworkEntries, ({ one }) => ({
  ...
  createdBy: one(users, { fields: [reworkEntries.createdByUserId], references: [users.id] }),
}));
...
export const insertRejectionEntrySchema = createInsertSchema(rejectionEntries).omit({ id: true, date: true, importedAt: true, createdByUserId: true });
export const insertReworkTypeSchema = createInsertSchema(reworkTypes).omit({ id: true });
export const insertReworkEntrySchema = createInsertSchema(reworkEntries).omit({ id: true, date: true, importedAt: true, createdByUserId: true });
