...
export async function registerRoutes(
  ...
  app.post("/api/members", isAdmin, async (req, res, next) => {
    try {
      const user = req.user as User;
      const { email, username, password } = z.object({
      ...
      const hashed = await hashPassword(password);
      const newUser = await storage.createUser({ email, username, password: hashed, organizationId: user.organizationId });
      const { password: _, ...safe } = newUser;
      res.status(201).json(safe);
      const org = await storage.getOrganizationById(user.organizationId);
      sendWelcomeEmail(email, username, password, org?.name ?? "your organisation").catch((err) => {
        console.error("[email] Failed to send welcome email:", err?.message ?? err);
        console.log(`[email] Member created but email unsent — share credentials manually: username="${username}" password="${password}"`);
      });
    } catch (err) {
      if (err instanceof z.ZodError) return res.status(400).json({ message: err.errors[0].message });
      next(err);
