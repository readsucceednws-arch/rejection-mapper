...
export async function sendWelcomeEmail(toEmail: string, username: string, password: string, orgName: string, fromEmail?: string): Promise<void> {
  const appUrl = getAppUrl();
  const fromAddress = fromEmail || process.env.RESEND_FROM_EMAIL || "onboarding@resend.dev";

  if (!resend) {
    console.log(`[email] No API key — credentials for ${toEmail}: username="${username}" password="${password}"`);
    return;
  }
  ...
  try {
    ...
    if (result.error) {
      ...
      console.log(`[email] Fallback credentials — username="${username}" password="${password}"`);
