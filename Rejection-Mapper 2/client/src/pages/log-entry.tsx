...
export default function LogEntry() {
  ...
  function resolveZone(val?: string | null): string | null {
    if (!val || val === "rejection" || val === "rework") return null;
    return val;
  }

  const selectedRejType = rejectionTypes?.find(t => t.id === newEntry.rejectionTypeId);
  const currentZone = resolveZone(selectedRejType?.type);
              ...
              <div className="border border-border/50 rounded-lg p-4 space-y-4 bg-muted/20">
                ...
                {entries.length > 0 && (
                  <div className="space-y-2 border-t border-border/50 pt-4">
                    {entries.map((entry) => {
                      const reason = rejectionTypes?.find(t => t.id === entry.rejectionTypeId);
                      return (
                        <div key={entry.id} className="flex items-center justify-between p-3 bg-background rounded border border-border/30">
                          <div className="flex-1">
                            <div className="text-sm font-medium">
                              ...
                              <span className="text-muted-foreground ml-2 text-xs">({entry.purpose})</span>
                            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {rejectionTypes?.filter(t => t.type === newEntry.purpose).map((type) => (
